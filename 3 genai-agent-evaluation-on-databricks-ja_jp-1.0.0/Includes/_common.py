...
class DBAcademyHelper(NestedNamespace):
    ...
    # 初期化子を追加。初期化子は連鎖可能で、DA.init()が呼ばれたときにすべて呼び出される。
    # このパターンにより、セルやノートブック間でクラスを動的に拡張することが容易になる。
    # これを使用する方法はいくつかあるが、関数デコレータとして使用するのが最も簡単：
    ...
    #
    # DA.init()が呼ばれると、すべての初期化子が追加された順序で呼び出される
    ...
    # クラスメソッドを追加（いわゆる「モンキーパッチ」）。このパターンにより、
    ...
    # SDKを介して一般的な検索を実行。例えば、名前を指定して構造のIDを見つける。使用例：
    # DA.workspace_find("catalogs", "main") -> "main"という名前のカタログを表すSDK構造を返す
    # DA.workspace_find("cluster_policies", "DBAcademy DLT") -> 名前付きポリシーを表す構造を返す
    # 注意：これが機能するためには、SDKに"item_type"で名前付けされたAPIと"list" APIが必要で、
    # "name"要素に基づいて検索したいと仮定している。しかし、これらの条件がすべて真でない場合は、
    # "member"と"api"を使用して独自の検索関数を実装することなく動作を調整できる。いくつかの例：
    # DA.workspace_find("clusters", "0913-023811-rzeq07rk", "cluster_id") -> 
    # "cluster_id"の一致する値を持つクラスター構造を返す
    # DA. workspace_find('pipelines', pipeline_name, api='list_pipelines') -> 
    # 名前付きDLTパイプラインを表す構造を返す
    def workspace_find(
        self,
        item_type: str,
        value: str=None,
        ...
        # API（アイテムタイプ）を見つけて、"list"メソッドを取得
    ...
    def display_config_values(self, config_values):
        """
        キーと値のペアのリストをHTMLテキストとテキストボックスの行として表示
